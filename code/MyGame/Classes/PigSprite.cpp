...
void PigSprite::f_followPlane(float dt) {
    ...
    float p = origin.x + visibleSize.width
    - plane->getContentSize().width * 0.7 / 2;
    
	if (PlanePos.x + 12 + spritepig->getContentSize().width * 0.6 <= p) {
		this->setPosition(Point(PlanePos.x + 12 + spritepig->getContentSize().width / 2,PlanePos.y));
	} else {
		this->setPosition(Point(PlanePos.x - 12 - spritepig->getContentSize().width / 2,PlanePos.y));
	}
